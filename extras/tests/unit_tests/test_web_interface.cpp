...
test_serves_correct_update_path()
{
  ...
  testHarness.serve_web_interface();
  assert(LogHasText("HTTP/1.1 200 OK\r\n", &netOut));
  assert(LogHasText("<b>Update server:</b> example.org", &netOut));
  assert(LogHasText("<b>Update file path:</b> /test", &netOut));
...
test_bad_path_404()
{
  ...
  // Serve webpage and check output
  testHarness.serve_web_interface();
  assert(LogHasText("HTTP/1.1 404 NOT FOUND\r\n", &netOut));
...
test_reset()
{
  ...
  request.Returns("readBytesUntil", 1, &three);
